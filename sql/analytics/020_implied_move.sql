WITH option_metrics AS (
    SELECT
        ...
        CASE
            WHEN side = 'Call' THEN
                iv / NULLIF((SELECT iv FROM optionchains AS o2
                             WHERE o2.side = 'Put'
                               AND o2.strike_price = o1.strike_price
                               AND o2.expiration_date = o1.expiration_date
                               AND o2.ticker_symbol = o1.ticker_symbol), 0)
            WHEN side = 'Put' THEN
                iv / NULLIF((SELECT iv FROM optionchains AS o2
                             WHERE o2.side = 'Call'
                               AND o2.strike_price = o1.strike_price
        ...
        (iv * SQRT((expiration_date::DATE - CURRENT_DATE)::DOUBLE PRECISION / 365)) /
        NULLIF(((bid + ask) / 2), 0) * 100 AS expected_move_percentage,
...
SET implied_move = om.implied_move
